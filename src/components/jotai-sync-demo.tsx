...
import {
  ...
  ssrSourceAtom,
  ssrTimestampAtom,
} from "@/components/server-data-store";
      ...
      {config ? (
        <div className="surface-100 border-round p-3">
          ...
          <div className="grid">
            <div className="col-12 md:col-6">
              <strong>Job Board Page Size:</strong>{" "}
              {String(config.job_board_page_size || "N/A")}
            </div>
            <div className="col-12 md:col-6">
              <strong>Job Board Radius:</strong>{" "}
              {String(config.job_board_recommendation_radius || "N/A")}
            </div>
            <div className="col-12 md:col-6">
          ...
          <div className="mt-3 text-xs text-500">
            <strong>Total Keys:</strong> {Object.keys(config).length}{" "}
            configuration items
          </div>
        </div>
      ) : (
        <div className="surface-100 border-round p-3 text-center">
          <span className="text-600">
            No configuration data available in client store
          </span>
        </div>
      )}
        ...
        content={
          <div>
            <p className="text-sm m-0">
              <strong>✅ Synchronized!</strong> This client component accesses
              the same config data that was stored in the server-side Jotai
              store and automatically hydrated to the client store.
            </p>
          </div>
        }
      />
        ...
        {ssrConfig ? (
          <div>
            ...
            <div className="mb-2 text-sm">
              ...
              {ssrTimestamp ? new Date(ssrTimestamp).toLocaleString() : "N/A"}
            </div>
            <div className="text-sm">
              <strong>Config Keys:</strong> {Object.keys(ssrConfig).length}{" "}
              items
            </div>
            <div className="mt-2 text-xs text-600">
              This data was hydrated from server-side SSR atoms using
              Jotai&apos;s official SSR utilities
            </div>
          </div>
        ) : (
          <div className="text-sm text-600">No SSR atom data available</div>
        )}
      </Card>
    </Card>
  );
